...
func teamAcceptSlashHandler(client *types.Client) func(s *discordgo.Session, i *discordgo.InteractionCreate) {
	return func(s *discordgo.Session, i *discordgo.InteractionCreate) {
		...
		_, err := client.HTBClient.Teams.AcceptInvite(client.Context, requestID)
		if err == nil {
			util.RespondEphemeral(s, i.Interaction, fmt.Sprintf("✅ Successfully accepted invite for request ID %d", requestID))
		} else {
			util.RespondEphemeral(s, i.Interaction, fmt.Sprintf("❌ Failed to accept invite for request ID %d: %v", requestID, err))
		}
	}
}
